...
import type { Task, SystemdUnit } from '@/shared/lib/types'
import {
  Play,
  Square,
  Trash2,
...
export function TasksPage() {
  const { server, api, serverId } = useServer()
  ...
  // Store api ref to avoid dependency issues
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      const data = await currentApi.listTasks()
      setTasks(data.tasks || [])
    } catch {
  ...
  const fetchSystemd = useCallback(async (mode: boolean) => {
    const currentApi = apiRef.current
    if (!currentApi) return
    try {
      const data = await currentApi.listSystemd(mode)
      setUnits(data.units || [])
    } catch {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    setError('')
    await Promise.all([fetchTasks(), fetchSystemd(userMode)])
    setLoading(false)
  }, [fetchTasks, fetchSystemd, userMode])
  ...
  }, [serverId]) // Only re-run when server changes

  // Fetch systemd when userMode changes
  useEffect(() => {
    fetchSystemd(userMode)
  }, [userMode, fetchSystemd])
